    ...
    const req = client.get(url, { timeout: timeoutMs }, (res) => {
      ...
      res.on("end", () => {
        try {
          ...
          reject(
            new Error(
              `Failed to parse SearXNG response: ${data.slice(0, 200)}`,
            ),
          );
        }
      });
...
export default function register(api: any) {
  ...
  const baseUrl =
    config.baseUrl ?? process.env.SEARXNG_BASE_URL ?? "http://localhost:8888";
  ...
  const tool = {
    ...
    description:
      "Search the web using SearXNG. Returns results from multiple search engines (Google, Bing, DuckDuckGo, etc.). Use this tool to find current information, research topics, or look up any facts on the internet.",
    ...
    async execute(
      ...
      try {
        const response = await fetchSearXNG(
          baseUrl,
          params.query,
          count,
          timeoutMs,
